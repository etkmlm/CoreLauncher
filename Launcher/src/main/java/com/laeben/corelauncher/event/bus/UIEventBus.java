...
public class UIEventBus<T extends EventContext, H extends BaseEvent<T, H>> extends IndirectEventBus<T, H> {
    ...
    protected boolean onEvent(Object clazz, EventRegister<T, H> register, H event, CancellableToken<?> cancellableToken) {
        if (register.isAsync()) UI.runAsync(() -> handle(clazz, register, event, cancellableToken));
