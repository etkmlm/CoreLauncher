package com.laeben.corelauncher.minecraft.wrapper.optifine;
...
public class OptiFine extends Wrapper<OptiVersion> {
    ...
    public OptiVersion getVersion(String id, String wrId) {
        ...
        var version = getAllVersions().stream().filter(x -> x.checkId(id) && x.getWrapperVersion().equals(wrId)).findFirst().orElse(null);
        if (version == null)
            return null;
    ...
    public List<OptiVersion> getAllVersions() {
        ...
        try{
            ...
            for (var a : all){
                ...
                OptiVersion v = new OptiVersion().setWrapperVersion(text);
                v.forgeWrapperVersion = forge;
                v.rawUrl = rawUrl;
                v.id = version;
