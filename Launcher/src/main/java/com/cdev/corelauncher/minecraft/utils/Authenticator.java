...
public class Authenticator {
    ...
    public String listen(String parameter) {
        ...
        var resource = Authenticator.class.getResourceAsStream("/com/cdev/corelauncher/data/auth.html");
        String r = resource == null ? null : NetUtils.inputStreamToString(resource).replace("$turnOff", Translator.translate("auth.ok"));

        String s = NetUtils.listenServer(LOCALHOST_PORT, r);
        if (s == null)
            return null;
        var m = p.matcher(s);
